...
/*   Updated: 2020/01/04 17:26:56 by astripeb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char				*get_s_filename(char *arg)
{
	...
	if (!(tmp = ft_strsub(arg, START, ft_strlen(arg) - EXTENSION)))
		return (NULL);
	if (!(new = ft_strjoin(tmp, ".s")))
	{
		free(tmp);
		return (NULL);
	}
...
int					get_num(int fd, size_t t_size, char type)
{
	...
	if (type == IND_CODE || t_size == IND_SIZE)
		num = (short)reverse_bits(num, t_size / sizeof(int));
	else if (type == DIR_CODE)
		num = reverse_bits(num, t_size / (sizeof(int)));
	return (num);
}
